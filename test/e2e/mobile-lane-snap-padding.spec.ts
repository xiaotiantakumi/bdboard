 ...
 * The base `.lanes-row` rule declares `scroll-snap-type: x proximity` and the `@media
 * (max-width: 700px)` block's `.lanes-row` override upgrades it to `x mandatory` on mobile, so
 * deleting the mandatory line leaves proximity snapping and this spec stays green. That is
 * intended — the contract this spec owns is "the lane snaps to the scroll-padding inset rather
 * than the scrollport edge", which both strengths satisfy at the offset used below. Replacing
 * scroll-snap-type with `none` (i.e. really removing the snap mechanism) does fail both
 * viewports, which is what makes this spec a real guard rather than the scrollIntoView version
 * it replaced. If the mandatory-vs-proximity distinction ever needs guarding, that belongs to a
 * spec about swipe behaviour, not this one.
 *
