...
(`KNOWN_SUB_AA_LIGHT` 参照)。20 → 18 は引き算だけでは合わないので順に:

| 段階 | 増減 | 件数 |
|---|---:|---:|
| bdboard-rr8m 時点 | | 20 |
| bdboard-skde が 3 件を解消 | -3 | 17 |
| opacity 合成の導入で disabled ボタン 4 件が新規検出 | +4 | 21 |
| その 4 件を `:disabled` 除外で落とす | -4 | 17 |
| `.tips-banner-text-body` を計上 | +1 | **18** |

途中の ±4 が相殺されるのが要点。ここを飛ばすと 20-3-4+1=14 という誤った数になる。
...
除外する。**除外の根拠は比率ではなく非活性であること**: トークンから計算すると
`.btn:disabled` は light 3.18:1 だが dark はダーク側のトークン上書きで 4.85:1 と AA を
満たすので、「どちらも 4.5 未満だから」という理屈は成り立たない。
...
新しく色を測るテストを書くときは、少なくともこの 8 つを踏まないこと。
...
5. **`Sample.color` は `opacity` を織り込まない。**(bdboard-97ib) `getComputedStyle().color`
   ...
   opacity 込みで測ると light は 3.18:1 になる (dark はダーク側トークンで 4.85:1 と AA を
   満たすので、除外の根拠は比率ではなく「非活性であること」そのもの)。

6. **`opacity` は前景と背景の比を保たない。掃引はそこを見ていない。**(bdboard-97ib)
   グループ `opacity` は前景・背景の両方を同じ式で真の backdrop へ寄せるので
   (`c' = a*c + (1-a)*B`)、輝度の大小は保たれるが**比は保たれない**
   (黒地に白 21:1 が `a=0.5`・B=黒 で 5.28:1 まで落ちる)。`cumulativeOpacity()` は
   実効背景を確定させるノードの**手前**までしか累積しないので、**不透明な背景を持つ要素
   自身に静止状態の `opacity` が付くと、掃引は実描画より良い比率を報告する**。
   今そのパターンは `.card.card-dragging` などドラッグ中の一時状態だけで静止掃引に
   掛からないが、静止状態でそれを足すなら前景と実効背景の両方に `opacity` を適用する形へ
   拡張すること。

7. **`background-image` / グラデーションは背景として見えない。**
   `getComputedStyle().backgroundColor` はグラデーションを返さないので、
   `linear-gradient` で塗られた面 (`.lane-scroll-indicator` など) は透明扱いで素通りし、
   その上のテキストは祖先の背景に対して測られる。現状そこにテキストは載っていない。
