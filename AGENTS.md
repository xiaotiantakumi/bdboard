...
- **フルチェーンは必ず `npm run verify` で回す。`npm run verify:steps` の直叩きは禁止** —
  プロセスグループ kill (bdboard-kia) とスロットの両方を迂回する。反復中の個別ステップは可。`lint` は ESLint + typescript-eslint (`src/` `web/src/` `scripts/` のみ、詳細は [docs/VERIFY.md](docs/VERIFY.md))。
...
- tsc 3 プロジェクトの表、slot の stale 処理と env ノブ、`npm run check:file-size` の baseline
  運用、`npm run check:commits` とコミットメッセージの括弧ガード: [docs/VERIFY.md](docs/VERIFY.md)。
...
- **止める・作り直しは `scripts/always-on-server.sh` 経由だけ (議長のみ)**: `BDBOARD_SERVER_CALLER=chair
  scripts/always-on-server.sh restart --expect-pid <PID>` (pull/build/health 待ち込み。PID は
  `status` で確認。cwd が古い worktree でも main の現在版へ exec し直す安全策あり。bdboard-9nah)。
  **Bash からの直接の停止コマンド (`kill`/`pkill`/`killall`) は `permissions.deny` で拒否される** —
  サブエージェントの main checkout pull/start/listener kill は hook 規則 7 も deny する (bdboard-hpu8)。
...
local-only and read once at startup). Original design doc: [docs/PLAN.md](docs/PLAN.md).
