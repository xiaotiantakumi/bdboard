...
export const BOARD_CHANGED_QUERY_KEY_EXCLUSIONS = {
  'ticket-runs':
    'エージェント実行の履歴 (bdboard 自身の runStore 由来)。.beads の変更とは無関係に変わるので、実行を開始・終了させた側が invalidate する: 詳細パネル自身の実行開始時と追跡中の実行の終了時、Next Up ループの実行開始時と終了時 (App が useNextUpRunLoopController に渡す通知)。',
  'agent-run':
