  ...
  it('appends cell-emptied warnings alongside the expired-exclude count', () => {
    expect(
      formatHarnessContractDetail({
        ...
        modelExclusionWarnings: ['models.routes.implement.low: 除外 (cursor) により候補が 0 件になりました (ラッパー scripts/aimix-run.sh は除外中の member の委譲を止め、他の member は表の判定なしで通します)'],
      }),
    ).toBe(
      '検証: npm run verify / PR 必須 / main: main / 期限切れの除外が 1 件 / ' +
        'models.routes.implement.low: 除外 (cursor) により候補が 0 件になりました (ラッパー scripts/aimix-run.sh は除外中の member の委譲を止め、他の member は表の判定なしで通します)',
    );
  });
});
  ...
  it('is true when a cell was emptied by exclusion, even though the contract is ok', () => {
    expect(
      harnessContractNeedsAttention({
        ...
        models: null,
        expiredExcludeCount: 0,
        modelExclusionWarnings: ['models.routes.review.low: 除外 (cursor) により候補が 0 件になりました (ラッパー scripts/aimix-run.sh は除外中の member の委譲を止め、他の member は表の判定なしで通します)'],
      }),
    ).toBe(true);
  });
});
