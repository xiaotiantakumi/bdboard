...
実測例(`web/src/components/ChatPanel.tsx`): 全体に対するシェアは 26%→16%→23%→9%→3% と推移して
おり、絶対件数だけを見ると「増えている」ように見えても実際はむしろ低下していた。

関連: チケットの起票根拠(epic の状態・関連 open チケットの有無)は1日で陳腐化しうる。投資判断
系のチケットを議論する前に `bd show` で前提を再確認すること。
