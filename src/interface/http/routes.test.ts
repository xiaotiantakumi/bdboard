...
describe('createApiRoutes', () => {
  ...
  it('returns 501 when sync health reader is not configured', async () => {
    const app = createApiRoutes(createDeps());
    const response = await app.request('/api/sync-health');
    const body = await response.json();

    expect(response.status).toBe(501);
    expect(body).toEqual({ error: 'sync health not available' });
  });

  it('returns sync health for all cached projects', async () => {
    const cache = createFakeBoardCache();
    ...
    const syncHealthReader: SyncHealthReader = {
      readSignals: vi.fn(async (rootPath: string) => {
        if (rootPath === '/projects/a') {
          return {
            localDoltRefHash: 'hash-a',
            localDoltRefCommitMs: 1_700_000_000_000,
            remoteDoltRefHash: 'hash-a',
            issuesJsonlMtimeMs: 1_700_000_000_000,
            interactionsUncommitted: false,
          };
        }
        return {
          localDoltRefHash: 'hash-b',
          localDoltRefCommitMs: 1_700_000_000_000,
          remoteDoltRefHash: 'other-b',
          issuesJsonlMtimeMs: 1_700_000_000_000,
    ...
    const app = createApiRoutes(createDeps({ cache, syncHealthReader }));
    const response = await app.request('/api/sync-health');
    const body = await response.json();
  ...
  it('filters sync health by projects query parameter', async () => {
    const cache = createFakeBoardCache();
    const projectA = project('proj-a', '/projects/a');
    const projectB = project('proj-b', '/projects/b');
    seedCache(cache, [
    ...
    const readSignals = vi.fn(async () => ({
      localDoltRefHash: 'hash',
      localDoltRefCommitMs: 1_700_000_000_000,
      remoteDoltRefHash: 'hash',
      issuesJsonlMtimeMs: 1_700_000_000_000,
      interactionsUncommitted: false,
    }));
    const syncHealthReader: SyncHealthReader = { readSignals };

    const app = createApiRoutes(createDeps({ cache, syncHealthReader }));
    const response = await app.request(
      `/api/sync-health?projects=${encodeURIComponent(projectA.id)}`,
    );
    const body = await response.json();
    ...
    expect(readSignals).toHaveBeenCalledWith('/projects/a');
  });

  it('tolerates per-project readSignals failures for sync health', async () => {
    const cache = createFakeBoardCache();
    const projectA = project('proj-a', '/projects/a');
    const projectB = project('proj-b', '/projects/b');
    seedCache(cache, [
