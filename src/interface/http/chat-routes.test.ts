...
// (1/3)。このPRでは GET /api/chat/availability・GET /api/chat/agents・chat agents
// rate limit/cache・POST /api/chat/message/stream を chat-agent-routes.test.ts /
// chat-message-stream-routes.test.ts へ抽出した。共有ヘルパー
// (createApp/createFakeBoardCache/withLocalHost 等) は chat-routes-test-support.ts
// へ移した。残りのテスト本体・期待値・モックの記述は元の chat-routes.test.ts から
// 一字一句変更していない。
