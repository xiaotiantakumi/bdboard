...
const BD_TOOL_USAGE_LINES: readonly string[] = [
  ...
  '- タイトル修正: bd_update_title',
  '- description 置換: bd_update_description',
  '- notes 追記(上書きではない): bd_append_notes',
  '- ブロック中でも bd 上の status は open のままになるので、',
  '  ブロック状況は bd_blocked または bd_show の依存関係で確認する',
  '- 新規チケット作成: bd_create',
  '- キーワード検索: bd_search',
  '- 顛末・経緯・過去の判断を問われたら、まず bd_search を使う。',
  '  bd_search は closed を含むため、履歴質問を open だけで探さない。',
  '- bd_list を使うときは status を明示する。open だけを見て「無い」と結論しない。',
  '- 1件見つけたら、親チケットと「導入したチケット/撤回したチケット」のような',
  '  対になるチケットも bd_show / bd_search で辿る。',
  '  例: 導入 bdboard-3tw.58 ↔ 削除 bdboard-3tw.151。',
 ...
 *
 * bd ツールの有無で出し分けない。根拠の集め方は変わる(MCP ツールか、シェルから
 * 直接 bd CLI か)が、「結論・根拠・食い違いの説明」という答えの形はどちらでも
 * 同じで、根拠なしの断定と曖昧な要約を防ぐのが目的だから。
 */
const OUTCOME_ANSWER_FORMAT_LINES: readonly string[] = [
  '「結局どうなった?」と顛末を問われたときの答え方:',
  '1. 結論を先に一文で。「直した」「対応不要にした」「削除した」「未着手」の',
  '   どれなのかを曖昧にしない。',
  '2. 根拠を添える。チケットID・コメントの日時・コミット/PR番号・実測値のうち',
  '   実際に見たものを書き、出典の無い経緯は書かない。',
  '   分からない部分は「bd 上に記録が無い」と明示する。',
  '3. ユーザーの観測(「まだ表示されている」等)と 1・2 が食い違うなら、',
  '   なぜ今そう見えているのかまで説明する。説明できないなら取り繕わず',
  '   「食い違っている」と言う。',
  '',
  '顛末を語る前提として押さえておくこと:',
  '- closed はマージされたことを意味しない。close は誰かが手で付けた印であって、',
  '  マージ状態から機械的に導かれるものではない。',
  '- 逆に、マージ済みでも画面には古いままが残ることがある。常時稼働の',
  '  サーバーが配信しているのはビルド済みの静的ファイルなので、',
  '  リビルドと再起動をしていなければ変わらない。',
  '- ボードの完了レーンには closed チケットのカードが残る。「まだ表示されている」が',
  '  機能のことではなくカードのことを指している可能性を先に潰す。',
];
