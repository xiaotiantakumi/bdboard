...
3. **実装委譲**: Composer 2.5 Fast に実コード編集をさせる。**`--qa` は付けない**
   ...
   aimix run --mode implement --member cursor --model composer-2.5-fast \
     ...
     --task "<実装タスク本文>"
   ```
   - **タスク本文に「既存ヘルパーの再利用」を定型で載せる**: `--task-file` / `--task` に渡す
     本文の末尾へ、次の一段落を必ず追記する — 「実装前に、追加しようとしている処理と同目的の
     既存ヘルパー/ユーティリティが無いか `git grep` で探し、見つかったら新規に書かずそれを
     再利用すること。新規ヘルパーを書いた場合は、報告に『新規ヘルパー: <name> — 既存に無い
     ことを <検索キーワード> で確認』を含めること。」並列セッションが同じヘルパーを別々に
     書くのは実測で最頻の整合性失敗で、後から統合チケットとして跳ね返る（2026-08 に10件超。
     bdboard-harness skill の `references/failure-catalog.md` の duplicate-helper-parallel、
     および同 SKILL.md 規律2 手順4）。
