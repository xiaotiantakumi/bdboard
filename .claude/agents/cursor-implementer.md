...
2. **タスク本文の確定（bdチケットIDを受け取った場合のみ。それ以外はこの手順をスキップ）**:
   ...
   # ↓ 既存ヘルパー再利用の定型。手順3 へ渡す前に必ずここで追記しておく
   cat >> "$REPO/.aimix-task-$ID.md" <<'EOF'

   ## 実装前チェック（必須）

   追加しようとしている処理と同目的の既存ヘルパー/ユーティリティが無いか `git grep` で探し、
   見つかったら新規に書かずそれを再利用すること。新規ヘルパーを書いた場合は、報告に
   「新規ヘルパー: <name> — 既存に無いことを <検索キーワード> で確認」を含めること。
   EOF
   ...
   - **上記の「実装前チェック（必須）」追記を済ませたうえで**、生成したファイルを次手順の
     `--task-file` に渡す。ファイル名がチケットID由来で対象repo/worktree内かつ一意なので、
     並列実行時のスクラッチパッド固定名衝突（実例: bdboard, 2026-08-15）が原理的に起きない。
     **完了報告の前に削除する。**
   - この追記が必須なのは、並列セッションが同じヘルパーを別々に書くのが実測で最頻の整合性失敗
     だから（2026-08 に統合チケットが10件超。bdboard-harness skill の
     `references/failure-catalog.md` の duplicate-helper-parallel、および同 SKILL.md 規律2 手順4）。
     bdチケット無しで `--task` に本文を直接渡す場合も、本文末尾へ同じ段落を必ず入れる。
...
5. **結果を返す**: 以下を構造化して呼び出し元に返す。**QAレビューはここでは行わない**
   ...
   - **evidence: bdboard-harness skill の `references/close-template.md` の4項目**を、自分が
     ...
     なら「未実行（議長が実行）」）/ `PR:`（このエージェントは PR を作らないので原則「なし」）/
     `レビュー:`（`--qa` を付けない方針なので原則「なし」）/ `未了:`（残作業。無ければ
     「なし」）。**やっていない項目を空欄にせず「なし」「未実行」と明示する** — 空欄は
     「やったが書き忘れた」と区別がつかず、議長が再検証するか信じるかの二択になる。
