...
6. 実装する。Codex に委譲する場合は `~/.agent/skills/ai-mix/SKILL.md` を Read し、aimix には `--model` を明示する (hook 規則6が照合)。
7. `docs/help-content.json` の追従が要るか確認し、結論を報告に書く。
8. `npm run drift` を実行し結果を確認する。
9. `npm run verify` を run_in_background で実行する (末尾に `&` を付けない。出力は `> logs/verify.log 2>&1` へ。sleep で待たず完了通知を待つ)。クリーンになるまで直す。`verify:steps` は直接叩かない。
...
14. `bd -C $MAIN comment <id> "PR: <url>"`。
