...
- **bdboard のスーパーバイザーは worktree-aware**（bdboard-6aci）。worktree かブランチが
  残っているチケットは `--id` 指定から外すので回収されない。よって heartbeat が一時的に
  途切れただけで作業を奪われることはない。ただし保護は**作業開始から 12 時間**で打ち切る
  （掃除し損ねた worktree が残っているだけのチケットが永久に in_progress で塩漬けになるのを
  防ぐため）。長丁場のチケットは heartbeat を切らさないこと。
