...
## 複数マウントされる hook でインスタンス間共有の関数参照を addEventListener に渡すと、1つしか登録されない

症状: 同じイベントリスナーを複数コンポーネント（同じ hook の複数インスタンス）が
それぞれ `addEventListener` で登録しているつもりが、実際には1つしか登録されて
おらず、最初にマウント解除されたコンポーネントの `removeEventListener` が、
まだマウントされている他コンポーネント分も含めて全員のリスナーを道連れに消す
（レビューで発見・修正: `useLaneStripHeightVar`、bdboard-1n0r / PR #319
review follow-up、2026-09-05。ミューテーションテストで再現・実証済み）。

原因: DOM 仕様の「同一の `(type, listener, capture)` の組は重複登録されない」
という規則。罠になるのは**インスタンスをまたいで共有される参照** — 具体的には
hook の外（module scope）で定義した関数や、親コンポーネントで一度だけ作って
props/context 経由で複数の子に配った関数。これをそのまま `addEventListener` に
渡すと、複数のコンポーネントインスタンスから見て「同じ listener」として扱われ、
ブラウザ側が2個目以降の登録を黙って無視する。エラーは出ない。

**`useCallback` は罠ではない** — hook 自身の中で `useCallback(fn, deps)` を
呼ぶ分には、マウントされたインスタンスごとに別々の関数オブジェクトが作られる
（fiber ごとにメモ化される）ため、インスタンス間で衝突しない。罠になるのは
あくまで共有参照であり、`useCallback` そのものを避ける理由にはならない。
...
module scope の関数や、親から配られた共有コールバックをそのまま渡すのは避ける
（意図が「複数箇所で同じ処理を呼びたい」であっても、実際の挙動は「ブラウザに
重複登録として弾かれる」なので食い違う）。

## パスの左省略表示に `direction: rtl` を使うと bidi 並べ替えで誤読を招く

症状: 長いファイルパスを CSS だけで左側省略（`...` を先頭に出す）しようとして
`direction: rtl` を当てると、375px 幅で実測した `/Users/takumi/src/private_src/
bdboard (v2)` が `...vate_src/bdboard (v2)/` のように表示され、先頭の `/` が
右端へ回り込んで末尾スラッシュに見える（実測: bdboard-h4xs.8、2026-09-05）。
パス自体を誤読させる。

原因: `direction: rtl` は段落の基底方向を右→左にする bidi
（bidirectional text）制御であって、省略記号の位置だけを動かす仕組みではない。
ラテン文字の並び（強い LTR の連続）は内部順序を保つので、パスの本体はそのまま
読める向きで表示される。罠になるのは、強い方向を持つ文字に挟まれていない
「方向中立」な文字（先頭の `/` など）で、これは基底方向（RTL）に解決され、
段落の始端＝右端へ移動する。結果として、本体はそのまま・先頭の `/` だけ右端に
出て末尾スラッシュのように見える、という誤読を招く見た目になる。
