...
- **検証より先に受領状態を1コミットする。** 委譲先の未コミット成果の上で実装を壊して戻す・
  rebase する・`git checkout -- <path>` / `git restore` / `git reset --hard` / `git clean` を
  打つと、自分の加えた分と区別されずに成果ごと消える。受領したら `git status --porcelain` を
  見て、未コミット差分があれば検証より先にコミットする（委譲先がコミット済みで差分が無ければ
  不要。外部 CLI 宛の委譲では「委譲ブリーフの git 制約」節の検知を先に済ませる）。HEAD へ
  戻す操作（検証のために壊して戻す一連）に入る前に `git status --porcelain -- <path>` が空で
  あることを確認し、戻す直前には `git diff -- <path>` が自分の加えた壊し分だけであることを
  確認してから戻す。作業ツリー全体に効く操作（`reset --hard` / `clean`）の前は、パス指定なしの
  `git status --porcelain` が空であることを確認する（実測: failure-catalog.md の
...
## 委譲先の「検証済み」申告は実測の有無で裏取りする

なぜ: 委譲先が一見成功して返っても、検証（vitest 等）を一度も実行できていないまま
「実測しました」と報告してくることがある。status=failed にはならず正常応答の体裁を
取るため、報告文だけ読むと未検証のまま採用してしまう。

- Cursor(composer-2.5) はレートリミットだけでなく、**シェルを一切実行できない状態**でも
  異常を報告せず「ファイル編集のみ・検証は未実行」を返しうる（2026-09-05, bdboard-h4xs.19。
  4 回の委譲すべてでこの状態になり、build / e2e / verify / 変異テスト / commit を全て
  呼び出し側が代行する羽目になった。委譲の目的（実装を隔離して任せる）が達成されず、
  検証まで呼び出し側の仕事として残るぶん往復コストがむしろ増える）。委譲先の報告に
  verify/e2e の **EXIT が実測値として含まれているか**を必ず確認し、含まれていなければ
  「委譲先が検証した」と読まない。この状態は品質の失敗ではなく**可用性の失敗**として扱い、
  同じセルの次候補へ切り替える（本則: model-routing.md の可用性フォールバック／
  `models.exclude`）。
- さらに一段深刻な亜種として、シェルが動かない状態の Cursor Composer が**他チケットの
  実測値を自分の実測結果として提示した**実例がある（2026-09-05, bdboard-z231）。
  `npx vitest` を一度も実行できていないのに、チケット本文に参考として載っていた別チケット
  （bdboard-d6b8）の実測表を「自分が計測した値」として提示し、その数値を根拠に14箇所の
  修正まで適用した。教訓は二方向:
  - **チケット本文に他チケットの実測値を書くときは「これは `<元チケットID>` の値であり、
    あなたの計測結果ではない」と明示する**（委譲先がそのまま自分の値として再提示する
    経路を塞ぐ）。
  - ミューテーション/実測を完了条件にしたチケットでは、報告される数値をそのまま採らず
    「どのコマンドを何回流したか」「ログはどこか」を報告形式に含めさせる。議長は最低
    1つの数値を独立に検算する。
- 両者に共通する対処: 委譲先の「全部通った」「実測しました」は自己申告として扱い、
  採用判定の前に検証コマンドを自分で回す（上の「独立検証」節の規律をそのまま適用する）。

## 裁定前に対象の範囲を自分で測り直す

なぜ: 実装者が提示した「問題の枠組み」をそのまま受け入れて裁定すると、対象そのものが
狭すぎたときに裁定ごと外れる。CI 緑・実装者の e2e 通過も、spec が測っていない失敗モードの
不在を証明しない。

実例（2026-09-05, bdboard-h4xs.19）: 実装者は「バーがレーンインジケータストリップに重なる。
許容範囲か裁定してほしい」と報告し、議長は一過性・z-index・代替案の劣後を根拠に「許容」と
裁定した。CI も全チェック緑、実装者の e2e もヒットテスト 6/6 通過。実際は 375x812 で
**可視レーン帯が 100% バーの下**、カードの選択チェックボックスは `elementFromPoint` が
バーのボタンを返して**タップ不能**だった。「ストリップとの重なり」ではなく「盤面全体の
被覆」であり、実装者の 6/6 は**バー自身のボタン**のヒットテストで、カード側は誰も
測っていなかった。

対策:

- 実装者が「これを許容してよいか」と聞いてきたら、**許容の可否より先に「影響範囲の定義が
  正しいか」を自分で測る**。報告された対象より広い範囲を1回測るだけで済む。
- 幾何・重なり系の裁定は議論でなく実測で決める。Playwright の一時 spec を worktree に
  置いて `elementFromPoint` と矩形を吐かせ、終わったら消す（数分で済む）。
- レビューは「指摘の緩和方向」だけでなく「**検査対象の欠落**」を必ず観点に入れる。

関連: 上の「委譲先の『検証済み』申告は実測の有無で裏取りする」（実装者の自己申告を
検証で裏取りする話の裏面 — こちらは議長自身の裁定の検証範囲の話）。

## 完了通知（completed）は最終報告とは限らない — 再委譲前に本文を読む

なぜ: サブエージェントの task-notification が status=completed で届いても「作業が終わった」
とは限らない。バックグラウンドジョブ（verify/Playwright 等）を投げてターンを終えただけの
場合、そのジョブの完了で同じ agentId が再起動されて作業を続ける（通知には
"same task-id may notify more than once" と明記されている — この文言自体が唯一の
in-band な手がかりになる）。これを取り違えて放棄と断定し、同じ worktree へ2体目の
エージェントを起動すると二重実装になる（1チケット = 1 worktree の原則、本則:
worktree-pr-flow.md 冒頭）。

実例（2026-09-06, bdboard-bdsd）: 実装エージェントが3回 completed で戻り、3回とも
中間報告（verify を待っている、等）だったのを放棄とみなし、同じ worktree へ2体目の
実装エージェントを起動した。2体目が読み取り段階だったため無傷回収できたが、編集段階まで
進んでいれば二重実装になっていた。

判別に使える信号と使えない信号（実測）:

- 使えない: pgrep / lsof によるプロセス確認 — bg ジョブの合間に見ると 0 件になり、
  「居ない」と誤判断する。
- 使えない: bd の lease — この経路のエージェントは heartbeat が打たれているとは
  限らない（実測では 46 分無し）ため、lease の残り時間は判定材料にならない。
- 使える: エージェント自身の報告テキスト。指定した報告項目が全部埋まった最終報告なのか、
  「〜を待っている」「continuing to work」等の中間状態なのかを読む。中間状態なら
  再起動される見込みが高いので再委譲しない。

再委譲する前に、報告本文が完了報告の体裁かどうかを必ず読むこと。

## 日付/時刻に触る変更は TZ=UTC でも回す

なぜ: 開発機の既定 TZ が `Asia/Tokyo` だと、「ホスト TZ へフォールバックする」ロジック
（`getBoardTimeZone()` 等）がローカルでは偶然 JST に解決し、JST 前提で書かれた既存
フィクスチャの期待値ズレを隠す。CI（GitHub Actions）は TZ=UTC で回るため、**ローカルが
緑のまま CI だけが落ちる**。「CI は UTC 前提」という原則自体は文書化済み（bdboard-ol9 /
bdboard-3tw.75）だが、見逃しの直接原因は原則の欠落ではなく、**TZ を明示して回し直す手順が
無かったこと**なので、ここには原則ではなく手順として書く。

規律:

- 日付・時刻の整形やタイムゾーンに関わるコード変更を独立検証するときは、**検証を回す側
  （依頼元／議長）が**、検証チェーン（規律4 手順1 の参照順で決めたコマンド）を **`TZ=UTC`
  を付けてもう一度ローカルで実行**し、CI と同じ TZ 条件で緑になることまで確認してから
  合格にする。委譲した読み取り専用レビュアーには回させない（下の「レビュー委譲時の制約」
  参照）。

```bash
# bash / zsh（例: 検証チェーンが npm run verify の場合）
npm run verify          # 開発機の TZ（例: Asia/Tokyo）
TZ=UTC npm run verify   # CI と同じ条件
# PowerShell: $env:TZ='UTC'; npm run verify
```

- `TZ=UTC` 側だけが落ちたら、テストを TZ 非依存に書き換えるのではなく、**期待値が前提と
  している TZ を明示的に固定する**（bdboard なら `setBoardTimeZoneOverride('Asia/Tokyo')`）。
  ホスト TZ へのフォールバックに乗ったままのテストは、実行環境が変わるたびに同じ穴を
  再現する。
- 実例（2026-08-29, bdboard-i759 / PR #156）: `dailyDigestMarkdown.test.ts` /
  `DailyDigest.test.tsx` / `HygienePanel.test.tsx` の 3 ファイルが上の明示固定を欠いたまま
  PR を開き、verify / verify-windows が failure になって初めて発覚した（マージ前に検知、
  実害なし）。ローカルでは 3 ファイルとも緑だった。

## レビュー委譲時の制約（読み取り専用レビュアーのブリーフ）

なぜ: レビュー用サブエージェントは「読んで指摘する」ためだけに起動する。書き込み権限を
与えると、レビュー中の副作用（気を利かせた修正コミット、他 worktree の掃除、`git stash`
による未コミット作業の消失）が指摘と混ざり、何が指摘で何が改変かを事後に切り分けられなく
なる。**この制約は担当モデルとは独立**で、モデルを上げても（bdboard では fable → opus）
据え置き。モデルが賢くなったから緩めてよい、という性質のものではない。

レビュー委譲のブリーフには、毎回この禁止事項リストをそのまま書く:
...
- サブエージェント起動禁止
- 検証チェーン（`npm run verify` 等）の実行禁止 — 検証は依頼した側が自分で回す
  （上の独立検証規律）。レビュアーが回すと検証スロットも無駄に食う
- `cloudflared` 起動禁止
- 報告の末尾で `git status --porcelain` を報告する（上の禁止が守られたことの証拠になる）
- 自分が触っていないファイルを片付けない
- 作業メモを書くなら `/tmp` 配下のみ
- hook の**回避経路の探索は対象外**（見つけても起票しない。brushup-protocol.md §5/§2 分類 E）

## 委譲ブリーフの git 制約 — 宛先別の定型

なぜ: git 操作の制約は、宛先によって逆向きの事故を生む。外部 CLI（Codex 等）は禁止を無視して
commit / push まで進みうる（下の「Codex 実装委譲が無断で commit / push …」節）。一方、
「push / PR 作成はしない」とだけ書くと、委譲先はコミットまで避けて成果を未コミットで残し、
議長の checkout / restore / rebase の巻き添えにする（failure-catalog.md の
delegation-brief-no-commit）。どちらの定型でも守る不変条件は「壊す・戻す前にコミット済み」。

- **宛先の区別は、実際に git を操作しうる主体で決める。** 議長が外部 CLI を直接呼ぶときと、
  外部 CLI を `aimix run` 等で中継するラッパー型サブエージェント（codex-implementer /
  cursor-implementer 等）へ渡すときは外部 CLI 宛を使い、ラッパーが外部 CLI へ渡すタスク本文
  にも同じ定型を入れる。自分で編集するセッション内サブエージェントにはセッション内
  サブエージェント宛を使う。
- 外部 CLI 宛は、封じ込め優先で**禁止形＋自己申告義務**を使う。「commit はしないでよい」の
  ような許可解除形は誤適用の余地を残す（実測ではこの形の指示が無視された）。定型文:
  「`git add` / `git commit` / `git push` / `gh` 系コマンドは一切実行しないこと。実行して
  しまった場合は、理由を問わず status=failed としてその旨を報告すること。」受領直後、
  Codex 無断 push 節の検知（`git log` / `git status` / `git ls-remote`）を済ませてから、議長が
  変更ファイルを明示して `git add <path>...` し、受領状態をコミットする（`git add -A` は
  使わない。検知より先に add すると、委譲先の無断 add の痕跡を吸収して見えなくなる）。
- セッション内サブエージェント宛は、定型「作業が一区切りしたら、自分が変更したファイルだけを
  明示して `git add <path>...` し、worktree でコミットまで行うこと（`git add -A` /
  `git add .`・`git push`・PR 作成・既存コミットの amend / rebase はしない）。コミットした
  SHA を報告に含めること。push や `gh pr create` 等を実行してしまった場合は status=failed
  として報告すること。」を使う。議長は `git log` を報告された SHA と突き合わせ、無断コミットと
  区別する。
- どちらの定型でも、独立検証節の「検証より先に受領状態を1コミットする」（受領時に未コミット
  差分があればコミット）は省略しない。
...
2. 応答が「実装担当に委譲しました」「担当に依頼しました」等の**間接表現**、または
   「N 個の独立作業に分けて実装を開始しました」等の**計画宣言のみ**で、自分が編集した
   という記述がない。
...
- **2 回連続**で同じ「0 編集 + 委譲文言」が出たら、それ以上リトライせず **status=failed として
  ...
  規律と同じ扱い）。
- **複数の独立作業を1つの統合ブリーフで渡すと、この症状が「N個の独立作業に分けて実装を
  開始しました」という計画宣言だけ返して0編集で終了する形でも再現する**（実測: 2026-09-05,
  bdboard-qxt1。9件の指摘対応を1ブリーフで渡したところ 68秒・編集0で終了。ブリーフを
  3分割し「計画宣言で終わるな。実際にファイルを編集して完了させろ」と明記して再実行したら
  正常に編集した）。Codex への委譲は**作業単位を分割して渡す**。1ブリーフに独立作業を
  詰め込みすぎない。ブリーフに「計画だけ返して終わるな。実際に編集を完了させること」を
  明記し、戻ってきたら**変更ファイル一覧が空でないこと**を必ず確認する。
...
- ブリーフの git 制約は、上の「委譲ブリーフの git 制約 — 宛先別の定型」節の**外部 CLI 宛**
  の定型（禁止形＋自己申告義務）で書き、受領後の議長コミットもその節の順序に従う。
