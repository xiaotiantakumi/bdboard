...
- 日付・時刻の整形やタイムゾーンに関わるコード変更をレビュー・独立検証するときは、
  検証チェーン（規律4 手順1 の参照順で決めたコマンド）を **`TZ=UTC` を付けてもう一度
  ローカルで実行**し、CI と同じ TZ 条件で緑になることまで確認してから合格にする。
...
- 他 worktree 不可触
- `git stash` 絶対禁止（他セッションの未コミット作業ごと退避してしまう）
