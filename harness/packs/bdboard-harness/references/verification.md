...
- Cursor(composer-2.5) はレートリミットだけでなく、**シェルを一切実行できない状態**でも
  ...
  呼び出し側が代行する羽目になった）。委譲先の報告に verify/e2e の **EXIT が実測値として
  含まれているか**を必ず確認し、含まれていなければ「委譲先が検証した」と読まない。
...
場合、そのジョブの完了で同じ agentId が再起動されて作業を続ける。これを取り違えて放棄と
断定し、同じ worktree へ2体目のエージェントを起動すると二重実装になる（1 worktree =
実装エージェント 1 体の原則違反）。
...
- 使えない: pgrep / lsof によるプロセス確認 — bg ジョブの合間に見ると 0 件になり、
  「居ない」と誤判断する。
- 使えない: bd の lease — この経路のエージェントは heartbeat を打たないため、
  lease の残り時間は判定材料にならない。
