...
| reclaim 猶予窓（`--older-than`） | **1チケットの実作業時間を上回る値（実測ベース。目安 1〜2 時間）。TTL の倍数で決めない** | 失効直後の回収は「一時停止しただけの生きた作業者」から仕事を奪う。**TTL×2 という尺度は「heartbeat が確実に打たれている」前提の上でしか意味を持たない**（2026-09-05: TTL 5分 + 猶予窓 10分 の構成で、生存セッションのチケット4件が claim の 15〜19 分後に回収された。bdboard-hybu）。猶予窓が守るべきなのは lease の倍数ではなく作業そのものの長さ |
...
- **bdboard のスーパーバイザーは worktree-aware**（bdboard-6aci）。`bd/<id>` ブランチか
  `.claude/worktrees/<id>` が残っているチケットは `--id` 指定から外すので回収されない。よって
  heartbeat が一時的に途切れただけで作業を奪われることはない。ただし条件が 2 つ付く。
  - 保護は**作業開始から 12 時間**で打ち切る（掃除し損ねた worktree が残っているだけの
    チケットが永久に in_progress で塩漬けになるのを防ぐため）。
  - **git を読めた巡回でしか効かない**。`git worktree list` / `git branch` が失敗した巡回は
    「worktree が無い」ではなく「分からない」として**そのプロジェクトごと見送る**ので、
    保護が外れて回収されることはないが、回収も進まない。
  この命名規約から外れたブランチ／worktree で作業しているなら保護は効かない。長丁場の
  チケットは保護を当てにせず heartbeat を切らさないこと。
  SKILL.md 規律2 手順5 の補足（本文から移設）:
...
- 手動実行してよいのは次が**全部**揃ったときだけ:
  1. スーパーバイザーが動いていない。
  2. 対象の lease が失効から猶予窓（上の表の値。TTL の倍数ではない）を十分過ぎている。
